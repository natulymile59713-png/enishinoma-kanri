...
function startAdminRealtime(){
  ...
  // 何もしなくても最新になるよう、ナビバッジを定期自動更新（Realtimeの取りこぼし対策）
  if(adminBadgeTimer){ clearInterval(adminBadgeTimer); }
  updateAllNavBadges();
  adminBadgeTimer = setInterval(updateAllNavBadges, 15000);
}
var adminBadgeTimer = null;
...
// セクション非表示時の軽量カウント更新
/** 変化時の更新：ダッシュボード表示中はフル再集計、それ以外はナビバッジのみ更新 */
...
/** ナビの各タブの未対応数バッジを最新化（軽量・どのセクションを開いていても実行）
 *  問い合わせ＝退会/転入を除いた未対応、申請＝卒業承認待ち＋退会open＋転入open */
async function updateAllNavBadges(){
  if(!currentAdmin) return;
  try{
    const [contactsRes, reportsRes, bookingsRes, cashbacksRes, sotsugyouRes, dmsRes, dmReviewsRes] = await Promise.all([
      ...
      supa.from('cashbacks').select('status'),
    ...
    const isApp = function(t){ return t === '退会申請' || t === '卒業生の間転入申請'; };
    const openContacts  = contacts.filter(c => c.status==='open' && c.contact_type!=='メッセージ' && c.contact_type!=='運営返信' && !isApp(c.contact_type)).length;
    const openMessages  = contacts.filter(c => c.status==='open' && c.contact_type==='メッセージ').length;
    const openWithdrawal= contacts.filter(c => c.status==='open' && c.contact_type==='退会申請').length;
    const openTransfer  = contacts.filter(c => c.status==='open' && c.contact_type==='卒業生の間転入申請').length;
    ...
    const pendingBookings = (bookingsRes.data || []).filter(b => b.status==='pending' && b.scheduled_date >= todayKey).length;

    const cbEligible = (cashbacksRes.data || []).filter(c => c.status==='eligible').length;

    // 卒業申請 承認待ちペア（両者 pending）
    const sgPairs = {};
    (sotsugyouRes.data || []).forEach(r => {
      ...
      if(r.user_id < r.partner_user_id) sgPairs[key].a = r; else sgPairs[key].b = r;
    });
    let awaitingApproval = 0;
    Object.keys(sgPairs).forEach(function(k){ var p = sgPairs[k]; if(p.a && p.b && p.a.status==='pending' && p.b.status==='pending') awaitingApproval++; });

    const reviewedIds = new Set(((dmReviewsRes && dmReviewsRes.data) || []).map(r => r.message_id));
    const dmFlagged = ((dmsRes && dmsRes.data) || []).filter(m => !reviewedIds.has(m.id) && detectProhibitedContent(m.body || '').length > 0).length;

    updateNavBadge('contacts', openContacts);
    updateNavBadge('messages', openMessages);
    updateNavBadge('dms', dmFlagged);
    updateNavBadge('reports', openReports);
    updateNavBadge('sotsugyou', awaitingApproval + openWithdrawal + openTransfer); // 申請＝卒業承認待ち＋退会＋転入
    updateNavBadge('bookings', pendingBookings);
    updateNavBadge('cashbacks', cbEligible);
  }catch(e){ console.log('updateAllNavBadges error:', e); }
}
...
/** 問い合わせ一覧を描画 */
/** 申請系（退会/転入）は「申請」ページへ集約するため問い合わせ一覧からは除外 */
const APPLICATION_CONTACT_TYPES = ['退会申請', '卒業生の間転入申請'];

/** 問い合わせ/申請の1行HTML */
function contactRowHtml(c){
  const statusClass = c.status === 'open' ? 'open' : 'replied';
  const rowClass = c.status === 'open' ? 'contact-row unread' : 'contact-row';
  return '<div class="'+rowClass+'" onclick="openDetail(\''+c.id+'\')">'
    + '<div class="contact-status-dot '+statusClass+'"></div>'
    + '<div class="contact-info"><div class="contact-meta">'
    + '<span class="contact-name">'+escapeHtml(c.nickname || '名無し')+'さん</span>'
    + (c.member_id ? '<span class="contact-id">'+escapeHtml(c.member_id)+'</span>' : '')
    + '<span class="contact-type-badge">'+escapeHtml(c.contact_type)+'</span>'
    + '</div><div class="contact-body-preview">'+escapeHtml(c.body)+'</div></div>'
    + '<div class="contact-time">'+formatRelative(c.created_at)+'</div></div>';
}

function renderContacts(){
  const list = document.getElementById('contacts-list');
  let filtered = currentFilter === 'all' ? allContacts : allContacts.filter(c => c.status === currentFilter);
  // 退会申請・転入申請は「申請」ページに集約（問い合わせには出さない）
  filtered = filtered.filter(c => APPLICATION_CONTACT_TYPES.indexOf(c.contact_type) < 0);
  if(filtered.length === 0){
    list.innerHTML = '<div class="empty-state">該当する問い合わせはありません。</div>';
    return;
  }
  list.innerHTML = filtered.map(contactRowHtml).join('');
}
...
async function openDetail(id){
  ...
  // 卒業生の間 転入申請 専用: 入金（3,690円）確認後、転入を承認
  if(c.contact_type === '卒業生の間転入申請' && c.user_id){
    html += '<div style="margin:1rem 0;padding:14px;background:rgba(122,92,192,.08);border:1px solid rgba(122,92,192,.3);border-radius:10px">';
    html += '<div style="font-size:12px;color:#7a5cc0;font-weight:500;margin-bottom:8px">🚪 転入手数料（3,690円）の入金を確認したら、卒業生の間への転入を承認できます</div>';
    html += '<button class="btn-send" style="background:#7a5cc0" onclick="approveVoiceTransfer(\'' + c.user_id + '\')">✅ 卒業生の間 転入を承認</button>';
    html += '</div>';
  }
...
/** 卒業生の間への転入を承認（voice_transfer_at をセット） */
async function approveVoiceTransfer(userId){
  if(!guardEdit()) return;
  if(!userId) return;
  if(!confirm('この方の卒業生の間への転入を承認しますか？\n（承認すると卒業生の間に参加できるようになります）')) return;
  try{
    const { error } = await supa.rpc('approve_voice_transfer', { p_user_id: userId });
    ...
    if(openContact){
      await supa.from('contacts').update({
        ...
        reply_text: '卒業生の間への転入を承認いたしました。卒業生の間へご参加いただけます。'
      }).eq('id', openContact.id);
    }
    alert('✓ 転入を承認しました');
    closeDetail();
    if(typeof loadApplications === 'function') loadApplications(); else loadContacts();
  }catch(e){
    console.log('approveVoiceTransfer error:', e);
    alert('エラーが発生しました');
  }
}
...
async function loadUsers(){
  ...
  try{
    ...
    const [usersRes, reportsRes, cancelReqRes, gradAppRes, reviewsRes, matchesRes, cashbackRes] = await Promise.all([
      ...
      supa.from('contacts').select('user_id, body, status, created_at').eq('contact_type', '退会申請').order('created_at', { ascending: false }),
    ...
    // 退会申請: open 集合（保留中バッジ用）＋ ユーザーごとの最新退会理由（卒業生/自主退会の振り分け用）
    const cancelRequestSet = new Set();
    const withdrawReasonByUser = {};
    if(cancelReqRes && cancelReqRes.data){
      // created_at 降順で取得済み → 各ユーザー最初に出てきたものが最新
      cancelReqRes.data.forEach(c => {
        if(!c.user_id) return;
        if(c.status === 'open') cancelRequestSet.add(c.user_id);
        if(!(c.user_id in withdrawReasonByUser)){
          withdrawReasonByUser[c.user_id] = parseWithdrawReason(c.body);
        }
      });
    }
    ...
    allUsers = (usersRes.data || []).map(u => {
      ...
      return Object.assign({}, u, {
        ...
        withdrawal_reason: withdrawReasonByUser[u.id] || '',
 ...
 *  ※アフィリエイト紹介の「卒業済」集計用。卒業認定(graduated_at)も含む。 */
...
/** 退会申請 contacts の本文から退会理由を抽出（「理由：◯◯」の1行目） */
function parseWithdrawReason(body){
  if(!body) return '';
  var m = String(body).match(/理由：(.+)/);
  return m ? m[1].trim() : '';
}
/** 退会理由が「カップル成立したため」か */
function isCoupleWithdrawal(u){
  return !!u && u.withdrawal_reason === 'カップル成立したため';
}
/** ユーザー一覧「卒業生」タブの判定。
 *  退会承認済(approved)で、かつ ①卒業鑑定を受けた(graduated_at) または ②退会理由が「カップル成立したため」 の人。
 *  在籍中(banned_at=null)で卒業認定済の人は「卒業準備中」扱いとし、卒業生に含めない。 */
function isGraduateListUser(u){
  if(!u || u.withdrawal_type !== 'approved') return false;
  return !!u.graduated_at || isCoupleWithdrawal(u);
}
/** ユーザー一覧「退会済」タブの判定（強制退会 + 自主退会）。
 *  退会済(banned_at あり)のうち、卒業生でない人すべて。 */
function isWithdrawnListUser(u){
  return !!u && !!u.banned_at && !isGraduateListUser(u);
}
/** ユーザーが「退会済」(運営処分)枠か判定 */
...
function updateUserCounts(){
  const active = allUsers.filter(u => !u.banned_at && !isGraduateListUser(u)).length;
  const banned = allUsers.filter(u => isWithdrawnListUser(u)).length;
  const graduated = allUsers.filter(u => isGraduateListUser(u)).length;
  // カップル: status_key=='coupled'（卒業準備中は別タブへ分離）
  const couple = allUsers.filter(u => !u.banned_at && !isGraduateListUser(u) && u.status_key === 'coupled').length;
  // 卒業準備中: status_key=='prep'（在籍中で卒業手続き中のユーザー。卒業認定済でも退会前なら準備中）
  const prep = allUsers.filter(u => !u.banned_at && !isGraduateListUser(u) && u.status_key === 'prep').length;
...
function renderUsers(){
  ...
  // ステータス絞り込み
  if(userStatusFilter === 'active') filtered = filtered.filter(u => !u.banned_at && !isGraduateListUser(u));
  else if(userStatusFilter === 'banned') filtered = filtered.filter(u => isWithdrawnListUser(u));
  else if(userStatusFilter === 'graduated') filtered = filtered.filter(u => isGraduateListUser(u));
  else if(userStatusFilter === 'couple') filtered = filtered.filter(u => !u.banned_at && !isGraduateListUser(u) && u.status_key === 'coupled');
  else if(userStatusFilter === 'prep') filtered = filtered.filter(u => !u.banned_at && !isGraduateListUser(u) && u.status_key === 'prep');
  // 登録種別 / キャッシュバックの絞り込み
  ...
  filtered.forEach(u => {
    ...
    var statusBadge;
    if(isGraduateListUser(u)){
      statusBadge = '<span class="user-status-badge graduated">卒業済</span>';
    }else if(isWithdrawnListUser(u)){
      // 退会済: 処分=強制退会 / 承認=自主退会
      var wLabel = (u.withdrawal_type === 'banned') ? '強制退会' : '自主退会';
      statusBadge = '<span class="user-status-badge banned">'+wLabel+'</span>';
    }else{
    ...
    html += '</div>';
    html += '</div>';
    // フリー/マッチング中/カップル成立/卒業準備中 のステータスバッジ
...
function buildUserDetailHtml(u){
  ...
  // 退会済ユーザーのステータス（卒業済 / 強制退会 / 自主退会）
  if(banned && !pureAff){
    var wLabel2, wColor2;
    if(isGraduateListUser(u)){
      wLabel2 = '卒業済';
      wColor2 = 'background:rgba(201,169,110,.18);color:#C9A96E;border:0.5px solid rgba(201,169,110,.45)';
    }else if(u.withdrawal_type === 'banned'){
      wLabel2 = '強制退会';
      wColor2 = 'background:rgba(192,80,80,.14);color:var(--red);border:0.5px solid rgba(192,80,80,.35)';
    }else{
      wLabel2 = '自主退会';
      wColor2 = 'background:rgba(192,80,80,.14);color:var(--red);border:0.5px solid rgba(192,80,80,.35)';
    }
    html += '<div class="detail-row"><div class="detail-label">ステータス</div><div class="detail-value"><span style="display:inline-block;padding:3px 10px;border-radius:10px;font-size:11px;font-weight:500;'+wColor2+'">'+wLabel2+'</span></div></div>';
  }
...
async function confirmBan(){
  ...
  try{
    ...
    // 退会申請(contacts)を承認済(replied)に → 申請ページの「承認待ち」から外れ「承認済」へ
    try{
      await supa.from('contacts').update({ status: 'replied', replied_at: new Date().toISOString() })
        .eq('user_id', openUser.id).eq('contact_type', '退会申請').eq('status', 'open');
    }catch(_){}
...
// ===== 申請ページ（卒業申請 / 退会申請 / 転入申請） =====
var appCategory = 'sotsugyou';   // 現在の申請種別
var appContactFilter = 'open';   // 退会/転入の承認待ち(open)/承認済(done)

/** 申請ページを読み込み（卒業申請＋問い合わせ＝退会/転入） */
async function loadApplications(){
  try{ await loadSotsugyouRequests(); }catch(e){ console.log('loadApplications sotsugyou error:', e); }
  try{ await loadContacts(); }catch(e){ console.log('loadApplications contacts error:', e); }
  updateAppCounts();
  renderApplications();
}

/** 申請種別の切替 */
function setAppCategory(cat){
  appCategory = cat;
  document.querySelectorAll('[data-appcat]').forEach(function(t){ t.classList.toggle('active', t.dataset.appcat === cat); });
  renderApplications();
}

/** 現在の申請種別を描画 */
function renderApplications(){
  var subf = document.getElementById('sotsugyou-subfilters');
  var csubf = document.getElementById('app-contact-subfilters');
  var sgList = document.getElementById('sotsugyou-list');
  var appList = document.getElementById('application-contact-list');
  if(appCategory === 'sotsugyou'){
    if(subf) subf.style.display = '';
    if(csubf) csubf.style.display = 'none';
    if(sgList) sgList.style.display = '';
    if(appList) appList.style.display = 'none';
    renderSotsugyouRequests();
  }else{
    if(subf) subf.style.display = 'none';
    if(csubf) csubf.style.display = '';
    if(sgList) sgList.style.display = 'none';
    if(appList) appList.style.display = '';
    renderApplicationContacts(appCategory === 'withdrawal' ? '退会申請' : '卒業生の間転入申請');
  }
}

/** 退会/転入の承認待ち/承認済 切替 */
function setAppContactFilter(f){
  appContactFilter = f;
  document.querySelectorAll('[data-acfilter]').forEach(function(t){ t.classList.toggle('active', t.dataset.acfilter === f); });
  renderApplications();
}

/** 退会申請 / 転入申請（contacts）の一覧描画。承認待ち=open / 承認済=それ以外 */
function renderApplicationContacts(type){
  var list = document.getElementById('application-contact-list');
  if(!list) return;
  var all = (allContacts || []).filter(function(c){ return c.contact_type === type; });
  // サブフィルタの件数
  var openCount = all.filter(function(c){ return c.status === 'open'; }).length;
  var doneCount = all.length - openCount;
  var oc = document.getElementById('account-open'); if(oc) oc.textContent = openCount;
  var dc = document.getElementById('account-done'); if(dc) dc.textContent = doneCount;
  // 承認待ち / 承認済 で絞り込み
  var rows = all.filter(function(c){ return appContactFilter === 'open' ? c.status === 'open' : c.status !== 'open'; });
  rows.sort(function(a, b){ return new Date(b.created_at) - new Date(a.created_at); });
  if(!rows.length){
    list.innerHTML = '<div class="empty-state">該当する申請はありません。</div>';
    return;
  }
  list.innerHTML = rows.map(contactRowHtml).join('');
}

/** 申請種別タブのカウント＋ナビバッジ更新 */
function updateAppCounts(){
  var paired = 0;
  try{ paired = buildPairs().filter(function(p){ return classifyPair(p) === 'paired'; }).length; }catch(e){}
  var wd = (allContacts || []).filter(function(c){ return c.contact_type === '退会申請' && c.status === 'open'; }).length;
  var tr = (allContacts || []).filter(function(c){ return c.contact_type === '卒業生の間転入申請' && c.status === 'open'; }).length;
  var e1 = document.getElementById('appcount-sotsugyou'); if(e1) e1.textContent = paired;
  var e2 = document.getElementById('appcount-withdrawal'); if(e2) e2.textContent = wd;
  var e3 = document.getElementById('appcount-transfer'); if(e3) e3.textContent = tr;
  var navb = document.getElementById('nav-badge-sotsugyou');
  if(navb){ var total = paired + wd + tr; navb.textContent = total; navb.classList.toggle('zero', total === 0); }
}
...
async function loadDashboard(){
  ...
  try{
    ...
    const openContacts = contacts.filter(c => c.status === 'open' && c.contact_type !== 'メッセージ' && c.contact_type !== '運営返信' && c.contact_type !== '退会申請' && c.contact_type !== '卒業生の間転入申請').length;
    const openMessages = contacts.filter(c => c.status === 'open' && c.contact_type === 'メッセージ').length;
    ...
    // 申請未対応＝卒業申請(承認待ちペア) ＋ 退会申請(未対応) ＋ 転入申請(未対応)
    const openWithdrawalApp = contacts.filter(c => c.status === 'open' && c.contact_type === '退会申請').length;
    const openTransferApp = contacts.filter(c => c.status === 'open' && c.contact_type === '卒業生の間転入申請').length;
    const appPending = awaitingApproval + openWithdrawalApp + openTransferApp;
    ...
    html += pendingItem('申請未対応', appPending+'件', 'sotsugyou', appPending > 0);
    ...
    // nav の各バッジを未対応数で更新（問い合わせ/申請の集約を含む共通ロジック）
    updateAllNavBadges();
