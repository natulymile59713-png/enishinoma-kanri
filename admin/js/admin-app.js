...
async function loadUsers(){
  ...
  list.innerHTML = '<div class="empty-state">読み込み中…</div>';
  try{
    // ユーザー / 通報 / 退会申請(open) / 卒業鑑定申込 / レビュー / マッチ の並列取得
    const [usersRes, reportsRes, cancelReqRes, gradAppRes, reviewsRes, matchesRes] = await Promise.all([
...
function buildUserDetailHtml(u){
  ...
  if(banned){
    ...
    var typeLbl = wType === 'approved' ? '● 退会承認済み（同電話・同メアド再登録可）' : '● 退会処分済み（同電話・同メアド再登録不可）';
...
// 退会処分: 同電話・同メアドで再登録不可
// 退会承認: 同電話・同メアドで再登録可（auth.users.email がアーカイブ化、本人は再ログイン不可）
...
function openWithdrawModal(type){
  ...
  if(noteEl){
    noteEl.textContent = isApprove
      ? '※ 退会承認: 同じ電話番号・メールアドレスでの再登録が可能になります。元アカウントの登録情報はアプリ内に残ります。'
